...
export async function taskProcessor(job: Job<TaskJobData>): Promise<void> {
  ...
  logger.info({ taskId, jobId: job.id, attempt: job.attemptsMade + 1 }, 'Worker processing task');
  try {
    ...
    logger.error({ taskId, dbErr }, 'Failed to set task status to processing in database');
    throw dbErr;
  }
  try {
    ...
    logger.warn({ taskId, attempt: job.attemptsMade + 1, error: workErr.message }, 'Task attempt failed');
    ...
    if (isLastAttempt) {
      ...
      logger.error({ taskId }, 'Task execution exhausted all retries. Job marked as failed.');
    } else {
...
async function executeWorkload(taskId: string, payload: Record<string, any>): Promise<void> {
